...
export function NewCampForm({ campProgram }: CampFormProps) {
  const [selectedCampPrice, setSelectedCampPrice] = useState(0);
  const [selectedCampId, setSelectedCampId] = useState("");
  const [isSizeChartOpen, setIsSizeChartOpen] = useState(false);
  const [step, setStep] = useState(1); // Track the step (1 = initial info, 2 = conditions)
  ...
  const onSubmit = async (data: FormValues) => {
    if (step === 1) {
      ...
      // Proceed with final submission (payment)
      try {
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 mb-6 text-gray-50">
        {step === 1 && (
          <>
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-2xl mb-4 font-bold">Available Dates:</FormLabel>
                  <FormControl>
                    <RadioGroup
                      onValueChange={(value) => {
                    ...
                    >
                      {campProgram.sessions.map((camp) => (
                        <FormItem key={camp.id} className="flex items-center space-x-3 space-y-0">
                          <FormControl>
        ...
        {step === 2 && (
          <>
            ...
            <FormField control={form.control} name="subscribeToProgram" render={({ field }) => (
              <FormItem className="flex items-center space-x-2">
                <FormControl>
                  <input
                    type="checkbox"
                ...
                <FormLabel className="text-sm">
                  Sign me up for the free 12-week program to maximize what I have learned in the Camp
                </FormLabel>
                <FormMessage />
              </FormItem>
            )} />
